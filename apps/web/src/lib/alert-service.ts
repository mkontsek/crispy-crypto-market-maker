...
export function deriveAlerts(params: {
    ...
    const { health, inventory, config, pnl, killSwitchEngaged, quotes } =
        params;
    const alerts: Alert[] = [];
    ...
    if (disconnected.length > 0) {
        const names = disconnected
            .map((h) => `${h.exchange} (${h.pair})`)
            .join(', ');
        alerts.push({
            id: 'exchange-disconnect',
            severity: 'critical',
            message: `Feed disconnected: ${names}`,
        });
    }
    ...
    if (stale.length > 0) {
        ...
        alerts.push({
            id: 'feed-stale',
            severity: 'warning',
            message: `Feed staleness >2 s: ${names}`,
        });
    }

    if (config) {
        const maxMap = new Map(
            config.pairs.map((p) => [p.pair, sizeFromFp(p.maxInventory)])
        );
        for (const inv of inventory) {
            ...
            if (ratio > 0.9) {
                alerts.push({
                    id: `inv-crit-${inv.pair}`,
                    severity: 'critical',
                    message: `${inv.pair} inventory at ${(ratio * 100).toFixed(0)}% of hard limit`,
                });
            } else if (ratio > 0.75) {
                alerts.push({
                    id: `inv-warn-${inv.pair}`,
                    severity: 'warning',
                    message: `${inv.pair} inventory at ${(ratio * 100).toFixed(0)}% of limit`,
                });
            }
        }
    }
    ...
    if (pnl) {
        ...
        if (adverse > 0.4) {
            alerts.push({
                id: 'adverse-high',
                severity: 'warning',
                message: `Adverse selection high: ${(adverse * 100).toFixed(1)}%`,
            });
        }
        ...
        if (fillRate > 0 && fillRate < 0.02) {
            alerts.push({
                id: 'fill-low',
                severity: 'warning',
                message: `Fill rate very low: ${(fillRate * 100).toFixed(2)}%`,
            });
        }
    }
    ...
    if (allPaused && !killSwitchEngaged) {
        alerts.push({
            id: 'all-paused',
            severity: 'warning',
            message: 'All pairs are currently paused',
        });
    }
