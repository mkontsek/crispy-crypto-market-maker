...
describe('fetchGeoForUrl', () => {
  ...
  it('uses the server geo route for URL lookups', async () => {
    ...
    await expect(fetchGeoForUrl('http://localhost:3110')).resolves.toEqual(payload);
    expect(fetchMock).toHaveBeenCalledWith('/api/geo?url=http%3A%2F%2Flocalhost%3A3110');
  });

  it('returns null when the server route is unreachable', async () => {
    vi.stubGlobal('fetch', vi.fn().mockRejectedValue(new Error('network error')));
    await expect(fetchGeoForUrl('http://localhost:3110')).resolves.toBeNull();
  });
});
