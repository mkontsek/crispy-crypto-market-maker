...
describe('deriveAlerts', () => {
    ...
    it('adds a critical alert for disconnected exchange health entries', () => {
        ...
        expect(
            alerts.some(
                (a) =>
                    a.id === 'exchange-disconnect' && a.severity === 'critical'
            )
        ).toBe(true);
    });
    ...
    it('adds a warning alert for stale feeds (>2000ms)', () => {
        ...
        expect(
            alerts.some(
                (a) => a.id === 'feed-stale' && a.severity === 'warning'
            )
        ).toBe(true);
    });
    ...
    it('adds a critical inventory alert when ratio exceeds 90%', () => {
        ...
        expect(
            alerts.some(
                (a) => a.id === 'inv-crit-BTC/USDT' && a.severity === 'critical'
            )
        ).toBe(true);
    });
    ...
    it('adds a warning inventory alert when ratio is between 75% and 90%', () => {
        ...
        expect(
            alerts.some(
                (a) => a.id === 'inv-warn-BTC/USDT' && a.severity === 'warning'
            )
        ).toBe(true);
    });
    ...
    it('adds warning for high adverse selection rate', () => {
        ...
        expect(
            alerts.some(
                (a) => a.id === 'adverse-high' && a.severity === 'warning'
            )
        ).toBe(true);
    });
    ...
    it('adds warning for very low fill rate', () => {
        ...
        const alerts = deriveAlerts({ ...baseParams, pnl });
        expect(
            alerts.some((a) => a.id === 'fill-low' && a.severity === 'warning')
        ).toBe(true);
    });
    ...
    it('adds warning when all pairs are paused and kill switch is not engaged', () => {
        ...
        expect(
            alerts.some(
                (a) => a.id === 'all-paused' && a.severity === 'warning'
            )
        ).toBe(true);
    });
