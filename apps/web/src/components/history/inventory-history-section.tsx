...
export function InventoryHistorySection({ rows }: { rows: DbInventory[] }) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Inventory History ({rows.length} snapshots stored)</CardTitle>
      </CardHeader>
                    ...
                    <tbody>
                      {history.map((row) => (
                        <tr key={row.id} className="border-b border-slate-800/30">
                          ...
                          <td className="py-1 pr-3 font-mono">{row.inventory.toFixed(4)}</td>
                          <td className="py-1 font-mono">{row.normalizedSkew.toFixed(3)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
