...
export const EventLogSection: FC<EventLogSectionProps> = ({
    ...
    useEffect(() => {
        ...
        if (
            ...
            entries.push(
                makeLogEntry(
                    connected ? 'info' : 'critical',
                    connected
                        ? 'Bot stream connected'
                        : 'Bot stream disconnected'
                )
            );
        }
        prevConnected.current = connected;
        ...
        if (
            ...
            entries.push(
                makeLogEntry(
                    ...
                    killSwitchEngaged
                        ? 'Kill switch ENGAGED — all quoting halted'
                ...
                {log.length > 0 && (
                    <div className="h-48 space-y-1 overflow-y-auto">
                        {log.slice(0, 100).map((entry) => (
                            ...
                            >
                                <span className="min-w-[5rem] text-slate-500">
                                    {entry.time}
                                </span>
                                <Badge tone={logLevelTone(entry.level)}>
                                    {entry.level}
                                </Badge>
