...
function DepthBar({
  ...
  return (
    <div className="absolute inset-y-0 right-0 z-0 opacity-20" style={{ width: `${pct}%` }}>
      <div
  ...
  return (
    <div className="flex h-full flex-col rounded-xl border border-slate-800 bg-slate-900">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 px-4 py-3">
        <span className="text-sm font-semibold text-slate-200">Order Book</span>
        <span className="font-mono text-xs text-slate-400">BTC / USDT</span>
      </div>

      {/* Column headers */}
      <div className="grid grid-cols-3 border-b border-slate-800 px-4 py-1.5">
        <span className="text-left font-mono text-[11px] text-slate-500">Price (USDT)</span>
        <span className="text-center font-mono text-[11px] text-slate-500">Size (BTC)</span>
        <span className="text-right font-mono text-[11px] text-slate-500">Total</span>
      </div>
      ...
      <div className="flex-1 overflow-hidden px-2">
        ...
        <div className="relative my-0.5 flex items-center gap-3 border-y border-slate-700 bg-slate-800/60 px-4 py-2">
          ...
          <span className="font-mono text-base font-bold text-slate-100">
            ${MOCK_MID_PRICE.toLocaleString("en-US", { minimumFractionDigits: 1 })}
          </span>
