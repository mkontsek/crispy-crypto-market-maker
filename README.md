...
1. Copy DB env templates from `packages/db`:

```bash
cp packages/db/.env.example packages/db/.env
cp packages/db/.env.prod.example packages/db/.env.prod
```
...
```bash
POSTGRES_HOST_PORT=55440 pnpm --filter @crispy/db dev:up
```
...
- `DATABASE_URL` (optional — only needed if you want the web app to serve
  history / analytics data; DB writes are performed by the bot, not the web app)
    ...
    - Store local/dev value in `packages/db/.env`, and production value in `packages/db/.env.prod` or the provider secret manager.
...
For demo storage control, append-only tables are capped at 50,000 rows each and
the oldest rows are automatically pruned after writes (`Fill`, `Quote`,
`Inventory`, `PnLSnapshot`, and `SystemLog`).
...
To limit demo DB growth, the bot also prunes each append-only table to the most
recent 50,000 rows.
